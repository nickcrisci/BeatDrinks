...
//Creates the JSON formatted response in case of a successfull request
const createResponse = function(user) {
    return {
        data: {
            type: "user",
            id: user.id,
            attributes: {
                name: user.name,
                musicPreferences: user.musicPreference,
                cocktailPreferences: user.cocktailPreferences
            }
        }
    };
}
