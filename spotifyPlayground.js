...
function spotifyRecommendationWithGenre_alpha(authOptions, musicPreferences, limitMarketGenres) {
    ...
    request.post(authOptions, (error, response, body) => {
        if (!error && response.statusCode === 200) {
            ...
            var options = {
                url: generateUrl(musicPreferences, limitMarketGenres, (err, result) => {
                    if (!err) {
                        ...
                        return `/error/${err}` // must be improved
                    }
                }),
                headers: {
            ...
            request.get(options, (error, response, body) => {
                // real logic must be implemented here
...
spotifyRecommendationWithGenre_alpha(defaultUserAuth, [], {limit: 6, market: "DE", genre_seeds: ["disney", "movies"]});
